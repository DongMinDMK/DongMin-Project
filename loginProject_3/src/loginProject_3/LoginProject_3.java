...
static String session;
	...
	void signMethod(Scanner scanner, ArrayList<Member> memberList, Scanner lineScanner) { //회원가입 함수
	...
	void searchMethod(Scanner scanner, ArrayList<Member> memberList) {
	...
	void listMethod(ArrayList<Member> memberList) {
		Member member = new Member();
		for(int i=0; i<memberList.size(); i++) {
	...
	void loginMethod(Scanner scanner, ArrayList<Member> memberList) {
		Member member = new Member();
	...
	void deleteMethod(Scanner scanner, ArrayList<Member> memberList) {
	...
	void updateMethod(ArrayList<Member> memberList, Scanner scanner, Scanner lineScanner) {
		Member member = new Member();
		for(int i=0; i<memberList.size(); i++) {
			member = memberList.get(i);
	...
	void logoutMethod(Scanner scanner) {
	...
	public static void main(String[] args) {
		...
		LoginProject_3 lp3 = new LoginProject_3();
		...
		while(true) {
			...
			if(menu.equals("R") || menu.equals("r")) { //회원가입
				lp3.signMethod(scanner, memberList, lineScanner);
			}
			 else if(menu.equals("S") || menu.equals("s")) { //ID 찾기
				lp3.searchMethod(scanner, memberList);
			}
				
			 else if(menu.equals("L") || menu.equals("l")) { //회원목록
				lp3.listMethod(memberList);
			}

			else if(menu.equals("I") || menu.equals("i")) { //로그인
				lp3.loginMethod(scanner, memberList);
					
			}
			
			else if(menu.equals("D") || menu.equals("d")) { //회원탈퇴
				lp3.deleteMethod(scanner, memberList);
			}
			else if(menu.equals("E") || menu.equals("e")) { //회원수정
				lp3.updateMethod(memberList, scanner, lineScanner);
			}
			else if(menu.equals("O") || menu.equals("o")) { //로그아웃
				lp3.logoutMethod(scanner);
			}

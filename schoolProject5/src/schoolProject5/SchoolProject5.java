...
class Register{
	...
	static ArrayList<Manager> managerList;
	Scanner lineScanner;
...
class Search{ 
	...
	void searchMethod() { //찾기 함수의 시작
		...
		for(int i=0; i<Register.professorList.size(); i++) {
			Professor professor = Register.professorList.get(i);
		...
		for(int i=0; i<Register.managerList.size(); i++) {
			Manager manager =Register.managerList.get(i);
...
class Delete{
	...
	void deleteMethod() { //삭제함수의 시작
		...
		for(int i=0; i<Register.professorList.size(); i++) {//교수
			Professor professor = Register.professorList.get(i);
		...
		for(int i=0; i<Register.managerList.size(); i++) { //관리자
			Manager manager = Register.managerList.get(i);
...
class List{
	...
	void listMethod() { //전체출력함수의 시작
		System.out.println("======전체출력======");
		for(int i=0; i<Register.studentList.size(); i++) {
			Student student = Register.studentList.get(i); //학생
			...
			Student student= Register.studentList.get(i);
					if(student.getIrum().equals(irumUpdate)) {
						...
						if(modify.equals("yes") || modify.equals("YES")) {
							...
							Register.studentList.set(i, student); //haksaList에 0번지에 있는 해시맵을 바꿈.
						}
			...
			if(professor.getIrum2().equals(irumUpdate)) {
					...
					String modify = scanner.next();
							if(modify.equals("yes") || modify.equals("YES")) {
								...
								Register.professorList.set(i, professor); //haksaList에 1번지에 있는 해시맵을 바꿈.
							}
							else if(modify.equals("no") || modify.equals("NO")) {
								System.out.println("원래 데이터로 되돌아갑니다.");
								continue;
				...
				if(manager.getIrum3().equals(irumUpdate)) {
					...
					String modify = scanner.next();
						if(modify.equals("yes") || modify.equals("YES")) {
							...
							Register.managerList.set(i, manager); //haksaList에 2번지에 있는 해시맵을 바꿈.
						}
